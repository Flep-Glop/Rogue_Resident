...
class SkillTreeEditor {
  ...
  _renderNodeEditor(node) {
    ...
    try {
      ...
      // Connections section
      const connectionsSection = this._createConnectionsSection(node);
      ...
      form.appendChild(connectionsSection);
   ...
   */
  _createConnectionsSection(node) {
   ...
   * @param {String} targetNodeId - ID of the target node
   */
  _removeConnection(sourceNodeId, targetNodeId) {
  ...
  _renderCanvas() {
    ...
    requestAnimationFrame(() => {
      ...
      // Clear or reuse elements for better performance
      this._clearOrReuseElements();
  ...
  _createNodeIcon(node, x, y, radius, fragment) {
    ...
    // Determine icon character (simplified version)
    const iconMap = {
      'atom': '⚛',
      'brain': '🧠',
      'radiation': '☢',
   ...
   * @private
   */
  _addNodeEventListeners() {
    // Find all node elements
    const nodeElements = this.nodesGroup.querySelectorAll('.node');
    ...
    nodeElements.forEach(node => {
      ...
      node.addEventListener('click', (e) => {
        e.stopPropagation();
        this._selectNode(node.dataset.nodeId);
      });
      ...
      node.addEventListener('mousedown', (e) => {
        ...
        e.stopPropagation();
        this._startNodeDrag(node.dataset.nodeId, e);
      });
      
      // Add contextmenu handler for creating connections
      node.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        e.stopPropagation();
        this._startConnectionDrag(node.dataset.nodeId, e);
      });
    });
  }
  ...
  _highlightNodeUnderCursor(event) {
    ...
    allNodeElements.forEach(el => {
      el.setAttribute('stroke-width', el.dataset.nodeId === this.selectedNodeId ? '4' : '2');
    });
    
    // Check if cursor is over a node
    const nodeElement = event.target.closest('.node');
    if (nodeElement && nodeElement.dataset.nodeId !== this.editingConnection.sourceNodeId) {
  ...
  // Node event handlers (stubs for compatibility)
